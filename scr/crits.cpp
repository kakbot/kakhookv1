...
void Draw()
{
    ...
    if (CE_GOOD(LOCAL_E) && CE_GOOD(LOCAL_W))
    {
        ...
        if (draw)
        {
            ...
            if (!re::C_TFWeaponBase::CanFireCriticalShot(RAW_ENT(LOCAL_W), false, nullptr) || !added_per_shot && HasWeapon(LOCAL_E, 56) || HasWeapon(LOCAL_E, 1005) || HasWeapon(LOCAL_E, 1092)
            {

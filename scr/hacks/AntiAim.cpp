...
bool CAntiAim::FindEdge(float edgeOrigYaw)
{
    ...
    if (edgeLeftDist >= 260)
    {
        edgeLeftDist = 999999999.f;
    }
    if (edgeRightDist >= 260)
    {
        edgeRightDist = 999999999.f;
    }

    // If none of the vectors found a wall, then dont edge
    if (Utils::CompareFloat(edgeLeftDist, edgeRightDist))
    {
        return false;
    }
    ...
    {
        edgeToEdgeOn = 1;
        if (Vars::AntiHack::AntiAim::Pitch.Value == 2 || Vars::AntiHack::AntiAim::Pitch.Value == 4 || G::RealViewAngles.x < 10.f) // Check for real up
        {
            edgeToEdgeOn = 2;
        }
        return true;
...
float CAntiAim::GetAngle(int nIndex)
{
    ...
    case 7:
    {
        if (edgeToEdgeOn == 1)
        {
            retnAngle = bPacketFlip ? 90.f : -90.f;
        }
        else if (edgeToEdgeOn == 2)
        {
            retnAngle = bPacketFlip ? -90.0f : 90.f;
        }
        break;
    }
    case 8:
...
void CAntiAim::Run(CUserCmd *pCmd, bool *pSendPacket)
{
    ...
    if (!Vars::AntiHack::AntiAim::Active.Value || G::ForceSendPacket || G::AvoidingBackstab)
    {
        return;
    }
    ...
    if (const auto &pLocal = g_EntityCache.GetLocal())
    {
        ...
        if (bYawSet)
        {
            *pSendPacket = bPacketFlip = !bPacketFlip;
        }
